...
- **Add-your-own-gym (2026-06-22):** gym dropdown is now rendered from `GYMS` (base) + a new persisted `custom_gyms` key (in `DATA_KEYS`, so it cloud-syncs). "➕ Add new gym…" opens `#modal-add-gym` (name + wall height → `saveNewGym`). `getWallHeight`/labels use `allGyms()`.
